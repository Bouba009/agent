        ...
        dbQuery {
            if (TirourdaProductsTable.selectAll().count() == 0L) {
                TirourdaProductsTable.insert {
                    ...
                    it[nameAr] = "عسل جبلي حر تيروردة 500غ"; it[category] = "Miel"; it[priceDzd] = 2800.0; it[stock] = 45; it[salesCount] = 135; it[trendStatus] = "RISING"; it[searchInterest] = 96
                }
                TirourdaProductsTable.insert {
                    it[id] = UUID.randomUUID(); it[sku] = "TIR-OLIVE-01"; it[nameFr] = "Huile d'Olive Vierge Extra Kabylie 1L"
                    it[nameAr] = "زيت زيتون بكر ممتاز تيروردة 1 لتر"; it[category] = "Huiles"; it[priceDzd] = 1600.0; it[stock] = 14; it[salesCount] = 240; it[trendStatus] = "RISING"; it[searchInterest] = 92
                }
                TirourdaProductsTable.insert {
                    it[id] = UUID.randomUUID(); it[sku] = "TIR-NIGELLE-01"; it[nameFr] = "Huile de Nigelle Pure Pressée à Froid 100ml"
                    it[nameAr] = "زيت الحبة السوداء معصور على البارد 100مل"; it[category] = "Huiles"; it[priceDzd] = 850.0; it[stock] = 70; it[salesCount] = 90; it[trendStatus] = "STABLE"; it[searchInterest] = 84
                }
            }
        }
    }
...
class ReportService(private val aiEngine: AiEngine) {
    ...
    suspend fun generateDailyBrief(): DailyBriefDTO {
        return DailyBriefDTO(
            ...
            marketStatus = "طلب مرتفع (+45% اهتمام)",
            topTrendingProduct = "عسل جبلي حر / Miel Pur",
            urgentFollowUps = 8,
            lowStockAlerts = listOf("زيت زيتون 1 لتر (14 متبقية)"),
            recommendedAction = "التواصل مع محلات العطارة والمنتجات الطبيعية في العاصمة ووهران لتقديم أسعار الجملة."
        )
    }
}
    ...
    <header class="border-b border-slate-800 bg-slate-900/90 backdrop-blur px-4 lg:px-8 py-3 flex justify-between items-center sticky top-0 z-50">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-400">استخبارات السوق • تنقيب المتاجر • إدارة العملاء المحتملين CRM</p>
            </div>
        </div>
    ...
    <script>
        ...
        function renderLeadsTable(leads) {
            ...
            if (!leads || leads.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" class="p-8 text-center text-slate-400">لا توجد محلات مطابقة. جرب البحث بكلمة عامة مثل "عسل" أو "عطارة".</td></tr>';
                return;
            }
        ...
        async function executeDiscoverySearch() {
            ...
            try {
                ...
                console.log("Discovery network error, using instant directory filtering");
            }
            ...
            if (filtered.length === 0) {
                // إذا كانت الكلمة عامة كالعطارة، نعرض كل من يطابق الكلمة دون التقييد بالولاية
